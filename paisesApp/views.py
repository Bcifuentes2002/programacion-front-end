...
def index(request):
    return render(request, "templatesPaises/index.html")

def chile(request):
    data = {
        "titulo": "Chile",
        "información": "Chile es un país largo y angosto que se extiende por el borde occidental de Sudamérica, con más de 6,000 km de costa en el océano Pacífico. Santiago, su capital, se ubica en un valle rodeado de los Andes y la cordillera de la Costa. En la Plaza de Armas de la ciudad, bordeada de palmeras, se encuentra la catedral neoclásica y el Museo de Historia Nacional. El enorme Parque Metropolitano cuenta con piscinas, un jardín botánico y un zoológico.",
       
    }
    return render(request, "templatesPaises/chile.html", data)

def argentina(request):
    data = {
        "titulo": "Argentina",
        "información": "Argentina es un país sudamericano de gran envergadura con un terreno que incluye las montañas de los Andes, lagos glaciales y praderas en las Pampas, la tierra tradicional de pastoreo de su famoso ganado. El país es conocido por el baile y la música del tango. Su gran capital cosmopolita, Buenos Aires, se centra en la Plaza de Mayo, rodeada por imponentes edificios del siglo XIX, como la Casa Rosada, el icónico palacio presidencial.",
       
    }
    return render(request, "templatesPaises/argentina.html", data)
...
def brazil(request):
    ...
    return render(request, "templatesPaises/brazil.html", data)
